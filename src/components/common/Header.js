...
export default function Header() {
  ...
  const [connect_label, setConnect_label] = useState("Connect to Wallet");
  ...
  const login = async () => {
    try{
      ...
      if (accounts[0] > 0)
      {
        setConnect_label(minimize(accounts[0]))
      } 
  ...
  const resourcesMenu = [
    ...
    { 
      ...
      link: '/assets/docs/BitCake White Paper .pdf',
      svgPath: '',
    },
    {
      value: 'Design an NFT',
    ...
    },
    {
      ...
      link: '/assets/docs/Become A Partner.pdf',
      svgPath: '',
    },
    // {
                  ...
                  <>
                    ...
                    <Disclosure.Panel>
                      {marketPlaceMenus.map((item, index) => {
                        <Disclosure.Panel>
                        return (
                          <Link to={item.link} key={index} className="bg-red submenu-wrapper">
