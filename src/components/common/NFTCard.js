...
export default function NFTCard(props) {
  ...
  const onClickFavourite = (e) => {
    ...
    isFavourite ? props.cardData.favourites -= 1 : props.cardData.favourites += 1
  };

  const style = {
    color: isFavourite ? "red" : "grey",
    fill : isFavourite ? "red" : "grey"
  }
        ...
        <div className="w-full bg-white flex px-4 items-center justify-between">
          <div className="flex items-center">
            <div className="text-gray-400 text-sm font-medium flex items-center">
              <HeartIcon className="w-4 h-4 mr-1" style={style} onClick={onClickFavourite} />
          ...
          <Link className="font-medium" to={`/category/${props.cardData.subCategory}`}>
          <div >{props.cardData.subCategory}</div>
          </Link>
        </div>
